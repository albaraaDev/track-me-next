...
import { CadencePreset, Tracker, cadencePresetSchema, trackerStatusSchema, trackerNotesSchema } from "@/domain/types";
...
const trackerFormSchema = z.object({
  ...
  activeWeekdays: z.array(z.number().min(0).max(6)).min(1, "اختر يوماً واحداً على الأقل"),
  items: z
    .array(
      z.object({
        id: z.string(),
...
const createId = () =>
  typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `id-${Date.now()}`;
...
export function TrackerCreateSheet({ projectId, sectionId, open, onOpenChange }: TrackerCreateSheetProps) {
  ...
  const updateItemLabel = React.useCallback(
    (id: string, label: string) => {
      form.setValue(
        "items",
        form.getValues("items").map((item) => (item.id === id ? { ...item, label } : item)),
      );
    },
    [form],
  );
  ...
  const addItem = React.useCallback(() => {
    form.setValue("items", [...form.getValues("items"), { id: createId(), label: "" }]);
  }, [form]);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="bottom"
        className="glass-panel max-h-[82vh] overflow-y-auto rounded-t-[2rem] border border-border p-6 pb-16 shadow-glow-soft sm:max-h-[75vh]"
      >
        <SheetHeader className="text-right">
          <SheetTitle>إنشاء جدول متابعة</SheetTitle>
          <SheetDescription>
            اختر نوع الجدول، عناصر المتابعة، ومدى الأيام الفاعلة. يمكنك تعديل كل التفاصيل بعد الإنشاء.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="mt-6">
          <Form {...form}>
            <form onSubmit={handleSubmit} className="space-y-5">
              <Tabs
                value={type}
                onValueChange={(value) => form.setValue("type", value as TrackerFormValues["type"])}
                className="rounded-3xl border border-border/60 bg-white/5 p-2 text-sm text-muted-foreground"
              >
                <TabsList className="grid w-full grid-cols-2 rounded-2xl bg-transparent">
                  <TabsTrigger
                    value="status"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="مثال: متابعة التمارين الصباحية" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea rows={3} placeholder="أضف وصفاً موجزاً لهذا الجدول" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input type="date" {...field} disabled={cadence !== "custom"} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      {[
                        ...
                        <Button
                          ...
                          onClick={() => field.onChange(option.value as CadencePreset)}
                        >
                          {option.label}
                        </Button>
                      ))}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="text-sm font-medium text-foreground">عناصر المتابعة</Label>
                  <Button type="button" size="sm" className="rounded-full" onClick={addItem}>
                    إضافة عنصر
                  </Button>
                </div>
              ...
              {type === "notes" ? (
                <div className="rounded-2xl border border-dashed border-border/60 p-3 text-xs text-muted-foreground">
                  <p>
                    عند إنشاء جدول ملاحظات، يمكنك تسجيل نص كامل لكل خلية مع إمكانية التحرير لاحقاً. ستعرض الخلايا
                    لمحة سريعة ويتم فتح التفاصيل عند الضغط المطوّل.
                  </p>
                </div>
              ) : (
                <div className="rounded-2xl border border-dashed border-border/60 p-3 text-xs text-muted-foreground">
                  <p>
                    جدول الحالات يتيح التبديل بين <strong className="text-status-done">تم</strong>،
                    <strong className="text-status-partial">جزئي</strong>،
                    <strong className="text-status-missed">لم يتم</strong>، أو إعادة التصفير عند الحاجة، مع دعم
                    الملاحظات للحالات الجزئية.
                  </p>
                </div>
              )}
